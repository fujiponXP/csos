...
import { JsonWindow } from "./ui/JsonWindow.js"; 
import { JsonUIBuilder, JsonObject } from "./ui/JsonParser.js";
      ...
      files.forEach((f) => {
        ...
        if (f.type === "file") {
          li.addEventListener("click", () => {
            ...
            fetch(`/api/file-load?file=${encodeURIComponent(filePath)}`)
              .then((r) => r.json())
...
document.addEventListener("click", (e) => {
  if (!menu.element.contains(e.target as Node) && e.target !== startBtn) {
    menu.hide();
  }
});
