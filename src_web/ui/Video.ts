...
export class Video {
  ...
  constructor(view: VideoView) {
    ...
    if (view.width) this.videoEl.width = view.width;
    if (view.height) this.videoEl.height = view.height;

    // JSON解析用に属性を保持
    this.element.dataset.objtype = "video";
    this.element.dataset.src = view.src;
    if (view.autoplay) this.element.dataset.autoplay = "true";
    if (view.controls !== undefined) this.element.dataset.controls = view.controls ? "true" : "false";
